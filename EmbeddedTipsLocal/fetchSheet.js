export default function fetchSheet() {
    ...
    */
    ...
    const q = 'Select A,B,C,D order by A desc LIMIT 1 Offset 2'; // Select all columns
    ...
    fetch(url)
        ...
        data.forEach((element, index) => {
            ...
            // Build innerHTML with property names and values, separated by newlines
            let innerHTML = '';
            for (const key in element) {
            if (element.hasOwnProperty(key)) {
                innerHTML += `${key}: ${element[key]}<br>`;
            }
            }
